...
<body>
    <%
        ...
        try {
            ...
            stmt = conn.prepareStatement("SELECT id, user_id, amount, payment_method, transaction_date, booking_id FROM Transactions WHERE status = 'failed' ORDER BY transaction_date DESC");
            rs = stmt.executeQuery();
    %>
    <h2>Failed Transaction Alerts</h2>
    <table border="1">
        <tr>
            <th>ID</th>
            <th>User ID</th>
        ...
        <tr>
            ...
            <td><a href="#">Retry</a> | <a href="#">Refund</a></td> <!-- Future links to servlets -->
        </tr>
        <%
            }
            if (!hasData) {
                out.println("<tr><td colspan='7'>No failed transactions alerts.</td></tr>");
            }
        %>
    </table>
    <%
            ...
            e.printStackTrace();
            out.println("<p>Error viewing alerts.</p>");
        } finally {
            if (rs != null) rs.close();
